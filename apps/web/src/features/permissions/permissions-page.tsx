...
function MemberPermissions({ member, projectId }: { member: z.infer<typeof projectMemberSchema>; projectId: string }) {
  ...
  const save = async () => {
    ...
    } catch { setState('idle'); toast.error(t('permissions.saveError')); }
  };
  return (
    <article className="px-4 py-5 sm:px-5">
      ...
      <fieldset className="mt-4"><legend className="sr-only">{t('common.permissions')}: {member.user.name}</legend><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{permissions.map((permission) => {
        ...
        return <label key={permission} className={cn('flex min-h-16 cursor-pointer items-start gap-3 rounded-md border p-3 transition-colors', checked ? 'border-primary bg-accent text-accent-foreground' : 'hover:bg-muted/50')}><input className="sr-only" type="radio" name={`permission-${member.user.id}`} value={permission} checked={checked} onChange={() => { setSelected(permission); setState('idle'); }} /><span className={cn('mt-0.5 grid size-4 shrink-0 place-items-center rounded-full border', checked && 'border-primary bg-primary text-primary-foreground')}>{checked ? <Check className="size-3" /> : null}</span><span><span className="block text-sm font-medium">{t(`permissions.permission.${permission}`)}</span><span className="mt-0.5 block text-xs leading-5 text-muted-foreground">{t(`permissions.permissionHint.${permission}`)}</span></span></label>;
      })}</div></fieldset>
      <div className="mt-4 flex flex-wrap items-center justify-end gap-3"><Button size="sm" onClick={() => void save()} loading={state === 'saving'}>{t('common.save')}</Button></div>
    </article>
  );
}
