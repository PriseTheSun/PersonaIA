...
export function PermissionsPage() {
  ...
  const projectsQuery = useApiQuery((signal) => workspaceId ? apiRequest(`/projects?workspaceId=${encodeURIComponent(workspaceId)}`, projectResponseSchema, { signal }) : Promise.resolve([]), [workspaceId]);
  const membersPath = workspaceId ? `/workspaces/${encodeURIComponent(workspaceId)}/members` : '';
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 rounded-lg border bg-card p-4 sm:grid-cols-2" aria-label={t('permissions.scopeTitle')}>
        <div className="space-y-2"><label htmlFor="permission-project" className="text-sm font-medium">{t('permissions.scope')}</label><select id="permission-project" value={selectedProjectId} onChange={(event) => setParam('project', event.target.value)} disabled={!workspaceId || projectsQuery.status !== 'success'} className="h-10 w-full rounded-md border border-input bg-card px-3 text-base disabled:opacity-60 md:text-sm"><option value="">{t('permissions.workspaceDefaults')}</option>{projectsQuery.status === 'success' ? projectsQuery.data.map((project) => <option key={project.id} value={project.id}>{t('permissions.projectOverride', { name: project.name })}</option>) : null}</select></div>
        <div className="space-y-2"><label htmlFor="permission-user" className="text-sm font-medium">{t('permissions.user')}</label><select id="permission-user" value={selectedUserId} onChange={(event) => setParam('user', event.target.value)} disabled={!workspaceId || membersQuery.status !== 'success'} className="h-10 w-full rounded-md border border-input bg-card px-3 text-base disabled:opacity-60 md:text-sm"><option value="">{t('permissions.selectUser')}</option>{membersQuery.status === 'success' ? membersQuery.data.map((member) => <option key={member.user.id} value={member.user.id}>{member.user.name} · {member.user.email}</option>) : null}</select></div>
      </section>
...
function PermissionMatrix({ member, workspaceId, projectId, explicitPermissions }: { member: z.infer<typeof permissionSubjectSchema>; workspaceId: string; projectId?: string; explicitPermissions: FunctionalPermission[] }) {
  const { t } = useTranslation();
