  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2"><Label htmlFor="access-status">{t('common.status')}</Label><select id="access-status" className="h-10 w-full rounded-md border border-input bg-background px-3 text-base md:text-sm" value={status} onChange={(event) => setStatus(event.target.value as 'ACTIVE' | 'SUSPENDED')}><option value="ACTIVE">{t('common.active')}</option><option value="SUSPENDED">{t('common.suspended')}</option></select></div>
        {isSuperAdmin ? <div className="space-y-2"><Label htmlFor="access-role">{t('accessControl.role')}</Label><select id="access-role" className="h-10 w-full rounded-md border border-input bg-background px-3 text-base md:text-sm" value={role} onChange={(event) => setRole(roleSchema.parse(event.target.value))}><option value="PROJECT_USER">{t('roles.PROJECT_USER')}</option><option value="CLIENT_ADMIN">{t('roles.CLIENT_ADMIN')}</option><option value="SUPER_ADMIN">{t('roles.SUPER_ADMIN')}</option></select></div> : null}
      </div>
      {isSuperAdmin && role !== 'SUPER_ADMIN' ? <div className="space-y-2"><Label htmlFor="access-tenant">{t('common.tenant')}</Label><select id="access-tenant" className="h-10 w-full rounded-md border border-input bg-background px-3 text-base md:text-sm" value={tenantId} onChange={(event) => setTenantId(event.target.value)}><option value="">{t('forms.selectTenant')}</option>{tenants.filter((tenant) => tenant.status === 'ACTIVE').map((tenant) => <option value={tenant.id} key={tenant.id}>{tenant.name}</option>)}</select></div> : null}
      {isSuperAdmin ? <div className="flex gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm leading-6 text-amber-950 dark:border-amber-900 dark:bg-amber-950/50 dark:text-amber-100"><ShieldCheck className="mt-1 size-4 shrink-0" aria-hidden="true" /><p>{t('accessControl.securityWarning')}</p></div> : null}
