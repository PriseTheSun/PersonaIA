...
export function UsersPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('users.title')} description={t('users.description')} action={<Button onClick={() => { setCreating(true); setMovingUserId(null); }}><Plus />{t('users.invite')}</Button>} />
      {creating && projectsQuery.status === 'success' ? <InlineForm title={t('forms.createUserTitle')} description={t('forms.createUserDescription')} onClose={() => setCreating(false)}><CreateUserForm projects={projectsQuery.data} onCancel={() => setCreating(false)} onCreated={() => { setCreating(false); toast.success(t('forms.created')); query.retry(); }} /></InlineForm> : null}
      {movingUser && projectsQuery.status === 'success' ? <InlineForm title={t('forms.moveUserTitle', { name: movingUser.name })} description={t('forms.moveUserDescription')} onClose={() => setMovingUserId(null)}><MoveUserForm userId={movingUser.id} memberships={movingUser.memberships} projects={projectsQuery.data} onCancel={() => setMovingUserId(null)} onMoved={() => { setMovingUserId(null); toast.success(t('forms.moved')); query.retry(); }} /></InlineForm> : null}
