...
export function CreateTenantForm({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<CreateTenantFormInput>({ resolver: zodResolver(createTenantFormSchema), defaultValues: { name: '', slug: '', segment: '', description: '', adminName: '', adminEmail: '', adminPassword: '' } });
  const submit = handleSubmit(async (input) => {
    setError(null);
    try {
      await apiRequest('/tenants', z.unknown(), { method: 'POST', headers: csrfHeaders(), body: { name: input.name, slug: input.slug, segment: input.segment, ...(input.description ? { description: input.description } : {}), admin: { name: input.adminName, email: input.adminEmail, password: input.adminPassword } } });
      onCreated();
    } catch (cause) { setError(cause instanceof Error ? cause.message : t('forms.error')); }
  });
  return <form onSubmit={submit} className="space-y-5" noValidate><MutationNotice message={error} type="error" /><div className="grid gap-4 sm:grid-cols-2"><FormField id="tenant-name" label={t('forms.tenantName')} error={errors.name && t(errors.name.message!)} {...register('name')} /><FormField id="tenant-slug" label={t('forms.slug')} error={errors.slug && t(errors.slug.message!)} placeholder="empresa-exemplo" autoCapitalize="none" {...register('slug')} /><FormField id="tenant-segment" label={t('forms.segment')} error={errors.segment && t(errors.segment.message!)} {...register('segment')} /><FormField id="tenant-description" label={t('forms.description')} error={errors.description && t(errors.description.message!)} {...register('description')} /></div><div className="border-t pt-5"><h3 className="mb-4 text-sm font-semibold">{t('forms.initialAdmin')}</h3><div className="grid gap-4 sm:grid-cols-2"><FormField id="admin-name" label={t('common.name')} error={errors.adminName && t(errors.adminName.message!)} {...register('adminName')} /><FormField id="admin-email" label={t('common.email')} type="email" autoComplete="username" error={errors.adminEmail && t(errors.adminEmail.message!)} {...register('adminEmail')} /><div className="sm:col-span-2"><FormField id="admin-password" label={t('common.password')} type="password" autoComplete="new-password" error={errors.adminPassword && t(errors.adminPassword.message!)} {...register('adminPassword')} /><p className="mt-2 text-xs leading-5 text-muted-foreground">{t('forms.passwordHint')}</p></div></div></div><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={onCancel}>{t('common.cancel')}</Button><Button type="submit" loading={isSubmitting}>{t('tenants.create')}</Button></div></form>;
}
