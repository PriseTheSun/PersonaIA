        ...
        <header className="sticky top-0 z-sticky flex h-16 items-center justify-between gap-2 border-b bg-background/95 px-3 backdrop-blur-sm sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-2 lg:hidden">
            <Dialog open={navigationOpen} onOpenChange={setNavigationOpen}>
              ...
              <DialogContent className="bottom-0 left-0 top-0 flex h-full w-[min(288px,calc(100%-2rem))] max-w-none translate-x-0 translate-y-0 flex-col rounded-none p-0">
                ...
                <div className="flex-1 overflow-y-auto px-3 py-4"><Navigation role={auth.user.role} onNavigate={() => setNavigationOpen(false)} /></div>
                <div className="border-t bg-background p-3 safe-bottom"><AccountMenu /></div>
              </DialogContent>
            </Dialog>
            <AppLogo compact />
          </div>
